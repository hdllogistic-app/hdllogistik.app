...
import {
  listShippingRatesService,
  createShippingRateService,
} from '@/modules/settings/services/shipping-rate-settings.service';
...
export async function GET(request: Request) {
  try {
    // Read access for OWNER, ADMIN, OPS
    await requireRole([
      USER_ROLES.OWNER,
      USER_ROLES.ADMIN,
      USER_ROLES.OPS,
    ...
    const result = await listShippingRatesService({ search, status });
...
export async function POST(request: Request) {
  try {
    ...
    // Mutation access for OWNER, ADMIN ONLY (OPS, FINANCE, DRIVER blocked)
    ...
    const result = await createShippingRateService(body, user.userId);
