...
export default function DriverProfilePage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        if (data.success && data.driverName) {
          setProfile({
            employeeName: data.driverName,
            loginId: 'Driver Account',
            role: 'DRIVER',
          });
        }
      } catch (err) {
  ...
  const handleLogout = async () => {
    if (!window.confirm('Keluar dari aplikasi Driver?')) return;
    try {
    ...
    <div className="space-y-5">
      ...
      <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl space-y-4 shadow-xl">
        <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
          <div className="w-12 h-12 bg-sky-500/20 text-sky-400 rounded-2xl flex items-center justify-center font-bold text-lg">
            <User className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-base font-bold text-white">
              {loading ? <Loader2 className="w-4 h-4 animate-spin text-sky-400" /> : profile?.employeeName || 'Driver'}
            </h2>
            <div className="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
              <span className="font-mono text-emerald-400 font-bold">Role: DRIVER</span>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-sm w-full p-5 space-y-4 shadow-2xl relative">
            <h3 className="text-sm font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-2">
              ...
              <span>UBAH PASSWORD AKUN DRIVER</span>
            </h3>
